...
FB.ContactForm = function() {
	...
	var lrl_formNewCount = 0; // increment for new fields
	...
	function addField () {
		try {
			lrl_formNewCount++;
			if (lrl_formNewCount <= maxNewFields) {
				// Add to preview
				jQuery('#fb-extra-fields').append('<div id="fb-new-field' + lrl_formNewCount + '" fieldid="' + lrl_formNewCount + '" class="fb-new-fields"><div class="fb-fields"><div id="' + lrl_formNewCount + '" class="fb-remove"></div><label fieldid="' + lrl_formNewCount + '" for="fb-field' + lrl_formNewCount + '"><span class="label-text">' + GrunionFB_i18n.newLabel + '</span> </label><input type="text" id="fb-field' + lrl_formNewCount + '" disabled="disabled" /></div></div>');
				// Add to form object
				fbForm.fields[lrl_formNewCount] = {
				...
				jQuery('#fb-field-id').val(lrl_formNewCount);
	...
	return {
		...
		init: function () {
			...
			var data = {
				action: 'lrl_form_shortcode_to_json',
