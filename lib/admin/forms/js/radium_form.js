...
FB.ContactForm = function() {
	...
	'action' : 'bootstrap_basic_form_shortcode',
	...
	var bootstrap_basic_formNewCount = 0; // increment for new fields
	...
	function addField () {
		try {
			bootstrap_basic_formNewCount++;
			if (bootstrap_basic_formNewCount <= maxNewFields) {
				// Add to preview
				jQuery('#fb-extra-fields').append('<div id="fb-new-field' + bootstrap_basic_formNewCount + '" fieldid="' + bootstrap_basic_formNewCount + '" class="fb-new-fields"><div class="fb-fields"><div id="' + bootstrap_basic_formNewCount + '" class="fb-remove"></div><label fieldid="' + bootstrap_basic_formNewCount + '" for="fb-field' + bootstrap_basic_formNewCount + '"><span class="label-text">' + GrunionFB_i18n.newLabel + '</span> </label><input type="text" id="fb-field' + bootstrap_basic_formNewCount + '" disabled="disabled" /></div></div>');
				// Add to form object
				fbForm.fields[bootstrap_basic_formNewCount] = {
				...
				jQuery('#fb-field-id').val(bootstrap_basic_formNewCount);
	...
	return {
		...
		init: function () {
			...
			var data = {
				action: 'bootstrap_basic_form_shortcode_to_json',
