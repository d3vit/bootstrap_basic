...
function bootstrap_basic_form_post_type_columns_filter( $cols ) {
	$cols = array(
		...
		'feedback_message'		=> __( 'Message', 'bootstrap' ),
		'feedback_date'			=> __( 'Date', 'bootstrap' )
	);
	...
	switch ( $col ) {
		...
		case 'feedback_message':
			...
			if ( $post->post_status == 'trash' ) {
				...
				echo '"><a title="';
				echo esc_attr__( 'Restore this item from the Trash', 'bootstrap' );
				...
				echo '">' . __( 'Restore', 'bootstrap' ) . '</a></span> | ';

				echo "<span class='delete'> <a class='submitdelete' title='";
				echo esc_attr( __( 'Delete this item permanently', 'bootstrap' ) );
				echo "' href='" . get_delete_post_link( $post->ID, '', true );
				echo "'>" . __( 'Delete Permanently', 'bootstrap' ) . "</a></span>";
?>
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as spam', 'bootstrap' );
				...
				echo '">';
				echo '<a class="submitdelete" title="' . esc_attr__( 'Trash', 'bootstrap' );
				echo '" href="' . get_delete_post_link( $post_id );
				echo '">' . __( 'Trash', 'bootstrap' ) . '</a></span>';
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as NOT spam', 'bootstrap' );
...
<?php
		...
		case 'feedback_date':
			echo get_the_date( __( 'Y-m-d @ g:i:s A', 'bootstrap' ) );
			break;
	}
}
...
function bootstrap_basic_form_ajax_spam() {
	...
	if ( $_POST['make_it'] == 'spam' ) {
		...
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __( 'You are not allowed to move this item out of the Trash.', 'bootstrap' ) );

		if ( ! wp_untrash_post($post_id) )
			wp_die( __( 'Error in restoring from Trash.', 'bootstrap' ) );

	} elseif( $_POST['make_it'] == 'trash' ) {
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __( 'You are not allowed to move this item to the Trash.', 'bootstrap' ) );

		if ( ! wp_trash_post($post_id) )
			wp_die( __( 'Error in moving to Trash.', 'bootstrap' ) );
	...
	if ( isset( $status['publish'] ) ) {
		...
		$status_html .= '>' . __( 'Messages', 'bootstrap' ) . ' <span class="count">';
	...
	if ( isset( $status['trash'] ) ) {
		...
		$status_html .= '>' . __( 'Trash', 'bootstrap' ) . ' <span class="count">';
	...
	if ( isset( $status['spam'] ) ) {
		...
		$status_html .= '>' . __( 'Spam', 'bootstrap' ) . ' <span class="count">';
