jQuery(document).ready(function(){
	...
	jQuery("img[src='']").attr("src", lrl_upload.url);
	
	jQuery('.lrl-opts-upload').click(function() {
		header_clicked = true;
	...
	jQuery('.lrl-opts-upload-remove').click(function(){
		$relid = jQuery(this).attr('rel-id');
		jQuery('#'+$relid).val('');
		jQuery(this).prev().fadeIn('slow');
		jQuery(this).prev().prev().fadeOut('slow', function(){jQuery(this).attr("src", lrl_upload.url);});
